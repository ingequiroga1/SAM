...
export class CustomerEditDialogComponent implements OnInit, OnDestroy {
	...
	createForm() {
		this.customerForm2 = this.fb.group({
	...
	isControlInvalid(controlName: string): boolean {
		const control = this.customerForm2.controls[controlName];
	...
	prepareCustomer(): CustomerModel {
		const controls = this.customerForm2.controls;
